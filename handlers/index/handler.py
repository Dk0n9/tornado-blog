...
class IndexHandler(base.BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        result = self._dbOperate.getPostsList(bool(self.current_user),
                                              num=self.getSetting.page_number, page=page)
...
class PostDetailHandler(base.BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        if not info:
            ...
            return None
        if info['post'].is_hidden:
            if not self.current_user:  # 无权限查看隐藏文章将返回404页面
                self.write_error(404)
                return None
        openGraphProtocol = {
            'title': info['post'].title + ' -- ' + self.siteTitle,
            'description': info['post'].summary,
            'published_time': self.functions.formatTime(info['post'].create_timestamp),
            'author': [info['post'].author],  # 目前数据库结构不支持多个author，现在这样做方便以后修改
            'url': info['post'].url,
            'tags': info['tags']
        }
